...
#title,url,thumbnailurl,views,uploaded,desc,channel.
  ...
  url = [video.find_element(by=By.ID,value='video-title').get_attribute('href') for video in videos]
  thumbnail_url = [video.find_element(by=By.TAG_NAME,value='img').get_attribute('src') for video in videos]
  channel_name = [video.find_element(by=By.CLASS_NAME,value='ytd-channel-name').text.strip() for video in videos]
  views = [video.find_element(by=By.ID,value='metadata-line').text.strip() for video in videos]
  description = [video.find_element(by=By.ID,value='description-text').text.strip() for video in videos]
